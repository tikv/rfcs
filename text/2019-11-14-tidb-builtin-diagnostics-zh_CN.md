...
- 系统表只包含当前节点信息，不能体现整个集群的状态，如：SLOW_QUERY, PROCESSLIST, Statement table。
...
- 引入第三方日志收集组件对所有节点的日志进行收集
    - 优势：统一的日志管理，可以长时间保存，并方便检索，同时多个组件的日志可以按照时间排序归并
...
- TiKV 和 TiDB 没有统一的方式，记住后容易忘记

针对以上两个问题，本提案中使用 SQL 触发采样并将采样数据转换为火焰图作为查询结果显示，这个不但能降低对外部工具的依赖，同时也极大的提升效率。所以各个节点需要实现采样数据采集功能，并能对上层输出指定格式的采样数据。暂定输出为 github.com/google/pprof 定义的 ProtoBuf 格式。
...
在完成前两层的基础上，由于任何一个 TiDB 实例均可以通过 HTTP API 或 gRPC Service 访问其他节点的信息，从而为单个 TiDB 实现集群 Global View，本提案中通过新建一系列相关系统表将采集到的集群信息向上层提供数据，上层包括不限于：
...
当前日志搜索需要登陆多台机器分别进行检索，并且没有简单的办法对多个机器的检索结果按照时间全排序，为了简化通过日志排查问题的方式以及提高效率，可以新建一个 `tidb_cluster_log` 系统表，通过 gRPC Diagnosis Service 的 `search_log` 接口，将日志过滤的谓词下推到各个节点，并最终按照时间进行归并，最终提高一个高效的全链路日志系统。
...
以前需要人工进行问题诊断的原因在于各个组件分散，数据源和数据格式异构，不便于通过程序化的手段进行集群诊断。通过前面几层提供的数据系统表，每一个 TiDB 节点都有了一个稳定的全集群 Global View，所以可以在这个基础上实现一个问题诊断框架，通过定义诊断规则能够快速发现集群的已有问题和潜在问题。
