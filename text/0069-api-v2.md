...
# example: config keyspaces set-metadata foo default-config.raw-client.default-ttl-secs 100000
>> config keyspaces set-metadata <keyspace name> <metadata-path> <metadata-value>

>> config keyspaces delete-metadata <keyspace name> <metadata-path>
...
4. Enable `API V2` in TiKV config file and restart TiKV. Or set by online config change API (Not proposed in this RFC, but is good to have).
...
3. Disable `API V2` in TiKV config file and restart TiKV. Or set by online config change API (Not proposed in this RFC, but is good to have).
