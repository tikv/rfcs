...
# Create a new keyspace. A new keyspace id will be allocated if not provided. 
# The provided id should not conflict with the existing keyspaces.
>> config keyspaces create <keyspace name> [--id <Keyspace ID>]

# Delete the keyspace meta on PD. Note that this action will not automatically delete the data on TiKV.
>> config keyspaces delete <keyspace name>

# Resorte the latest deleted keyspace. 
# The lastest deleted keyspace whose id is not found in current keyspace list will be restored.
