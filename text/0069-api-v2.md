...
#### Data structure in `etcd`
  - Keyspace id: path: `/pd/keyspaces_alloc_id`, integer value, starting from `1`.
...
2. Acquire a new `/pd/keyspaces_alloc_id`.
```sh
➜ get /pd/keyspaces_alloc_id
1
➜ txn: cmp: val("/pd/keyspaces_alloc_id" = "1", then put /pd/keyspaces_alloc_id 2
```
...
Add a new parameter `keyspace_alloc_id` besides `cluster_id` and `alloc_id`.
