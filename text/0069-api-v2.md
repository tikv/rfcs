...
On adding/deleting keyspaces, `PD` update data in `etcd` first, and then update data in local cache.

Every 60 seconds, every `PD` loads all keyspaces from `etcd`, and stores in local cache (as a hash map), to avoid inconsistency caused by leader transfer. *(So it is possible for a client to get a deleted keyspace in maximum 6 minutes in case deleting and `PD` leader transfer happen at the same time.)*
