...
3. Flashbacking keyspace:<br />
  3.1. Flashbacking keyspace only affects the metadata, turning the keyspace name and id mapping to viable by clients.<br />
...
#### Data structure in `etcd`
  - Keyspace id: path: `/keyspaces_next_id`, integer value, starting from `1`.
  - Keyspaces: path: `/keyspaces/{name}`, value: `{ "id": <keyspace id>, "properties": { <properties json> } }`.
  - Deleted keyspaces: path: `/keyspaces_deleted/{id}_{name}`, same value as normal keyspace.

#### Initialization
In loading loop of `PD` leader (see "3. Local cache in `PD`"), when it is found that `/keyspaces_next_id` and `/keyspaces` are not existed, `PD` leader will put `/keyspaces_next_id = 1` and `/keyspaces/default = {"id": 0}`.

#### Local Cache in `PD`
As most of requests are reading, and adding new keyspace should be infrequent, we use local cache in `PD` for efficiency.

On becoming leader, leader `PD` reloads all keyspaces from `etcd`, and stores in local cache (as a hash map).

On adding/updating/deleting keyspaces, `PD` update data in `etcd` first, and then update data in local cache.

#### Get a Single Keyspace or All Keyspaces
Get from local cache in `PD`.

#### Add New Keyspace
1. Check uniqueness of keyspace name in local cache.
2. Acquire a new `keyspaces_next_id`.
```sh
➜ get /keyspaces_next_id
1    
➜ txn: cmp: val("/keyspaces_next_id" = "1", then put /keyspaces_next_id 2
```
...
Implement by Get, Update, and Write back.

#### Delete
```sh
➜ get /keyspaces/feature-store
{"key":"/keyspaces/feature-store","version":1,"value":"{\"id\":1}"}
➜ txn: cmp ver("/keyspaces/feature-store") = "1" then put /keyspaces_deleted/1_feature-store {"id":1}, del /keyspaces/feature-store
```

#### Flashback
```sh
➜ get /keyspaces_deleted/ --prefix --sort-by="CREATE" --order="DESCEND" --limit=1
[{"key":"/keyspaces/1_feature-store","version":1,"value":"{\"id\":1}"}]
➜ txn: cmp ver("/keyspaces/1_feature-store") = "1" && create("/keyspaces/feature-store") = "0" then put /keyspaces/feature-store {"id":1}
```
