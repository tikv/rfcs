...
Timestamp is 64 bits, composed of physical part with 46 bits, and logical part with 18 bits. 18 bits are large enough for logical part, and this format is just the same as TSO of [PD](https://github.com/tikv/pd), as well as other timestamps (e.g. `start_ts` and `commit_ts`) in TiKV, which will help to reuse logics on different kinds of timestamps. 
...
After successful initialization, HLC can tolerate fault of TSO. RawKV operations are normal, but time-related metrics such as RPO is not reasonable. Meanwhile, as TiKV-CDC being highly dependent to PD (for cluster and task management, checkpoint, etc.), the replication will not be working.

##### 2.2.2 Logical Part of Timestamp

Just as the [paper](https://cse.buffalo.edu/tech-reports/2014-04.pdf) said, logical part (i.e. `logical clock` in paper) is a counter, increased when a write operation happens after another, to indicate the causality.

As TiKV is a distributed system, in every store we maintain an instance of the counter. As it is only required to keep causality among requests of every key (see [2.1 Requirement](#2.1 Requirement)), every key exists in only one region, and only one peer (the leader) in the region can write, it's safe to generate timestamps locally in each store, in most cases.
...
Suppose the leader of a region was on store 1 at the beginning. When the first PUT happened, it got a lc as 10, and LC of store 1 was advanced to 11. When the leader transfer happened to store 2, we should take the LC of store 1 to store 2 (as LC of store 2 is smaller), otherwise the second PUT would get a lc as 8, and violate the causality.

This logic will be implemented in observer to avoid interfering with leader transfer procedure: every peer observes the raft message applied and keeps the maximum timestamp of the region (called `Region-Max-TS`). On becoming leader, the peer advances the logical part counter of its store to be larger than `Region-Max-TS`. 

Besides, other region changes should be handled:
...
The *garbage collection* of deleted data is implemented by setting TTL to a lifetime defaults to 24 hours. When expired, the deleted data will be physically deleted by [compaction filter](https://github.com/facebook/rocksdb/wiki/Compaction-Filter). At the same time, start timestamp (i.e. `start-ts`) of **TiKV-CDC** replication task should not be earlier than the lifetime, otherwise the replication will miss some deletion events.
