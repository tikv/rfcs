...
Moreover, the logical part is advanced on *Leader Transfer*. As TiKV is a distributed system, every store in the TiKV cluster has an instance of timestamp generator. As every key exists in only one region and only one peer (the leader) in the region can write, it's safe to generate timestamps locally in each store, except for leader transfer.

On leader transfer, the timestamp generated by the original store could be larger than the store where the new leader is located, which will violate the causal consistency requirement. We solve this by this method: Every peer observes the raft message applied and keeps the maximum timestamp of the region (called `Region-Max-TS`). To become leader, this peer advances the timestamp generator of its store to be larger than `Region-Max-TS`. 
