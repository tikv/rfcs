...
为了减少内存使用，对于时间轴采用可配置的分层压缩，目前分为 3 层：
...
- 按 `Key` 数量 （Google 采用）：行数量少的 `Region` 统计信息合并取加权平均值：
...
因此，考虑将数据持久化到目前 `PD` 的 `leveldb` 中（后续可以也考虑存到 tikv 中）：
...
- 根据请求的 `startTime` 和 `endTime` 确定时间轴上列的数量和每个 key 轴的 [StartTime, EndTime)
